...
- **Signature** : sections en aplat bleu profond bordées de vagues organiques (SVG en `--wave`), images et portraits en arche (`.photo-arch`), cartes d'accompagnements en teintes variées, motif d'anneaux pointillés, et **bandeau bleu en vague derrière le portrait du hero** (`.hero-band`, SVG inline en absolu dans l'angle supérieur gauche de `figure.portrait`, sous la photo qui garde le z-index) : il déborde à gauche et dans le creux de l'arche, son bord supérieur descend vers la droite pour rester caché derrière la photo.
